(function() {
  ...
  /* ── Generate unique token for cart / wishlist ═── */
  ...
  function updateCartBadge() {
    ...
    .then(function(d) {
      ...
      var count = items.reduce(function(sum, i) { return sum + (i.quantity || 0); }, 0);
  ...
  document.addEventListener('click', function(e) {
    const btn = e.target.closest('.prod-act');
    if (!btn) return;
    ...
    if (action === 'addtocart') {
      ...
      const cartToken = localStorage.getItem('cart_token') || generateToken();
      localStorage.setItem('cart_token', cartToken);

      fetch('/api/cart/add', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-Cart-Token': cartToken },
        body: JSON.stringify({ product_id: productId, variant_id: variantId || null, quantity: 1, token: cartToken })
      })
      .then(function(r) { return r.json(); })
      .then(function(d) {
        if (d.success) { updateCartBadge(); showSuccessToast('Added to cart'); }
        else { showErrorToast(d.error || 'Could not add to cart'); }
      })
      .catch(function() { showErrorToast('Could not add to cart'); });
    }
    else if (action === 'wishlist') {
      const wToken = localStorage.getItem('wishlist_token') || generateToken();
      localStorage.setItem('wishlist_token', wToken);

      fetch('/api/wishlist', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-Cart-Token': wToken },
        body: JSON.stringify({ product_id: productId, token: wToken })
      })
      .then(function(r) { return r.json(); })
      .then(function(d) {
        if (d.success) { showSuccessToast('Added to wishlist'); }
        else { showErrorToast(d.error || 'Could not add to wishlist'); }
      })
      .catch(function() { showErrorToast('Could not add to wishlist'); });
    }
  });
  ...
  function renderNavAuth() {
    ...
    if (window.FV.user) {
      const name = window.FV.user.first_name || window.FV.user.email.split('@')[0];
      area.innerHTML = '<span class="nav-link-auth" id="nav-auth-trigger" tabindex="0" style="cursor:pointer">' + escHtml(name) + '</span>';

      if (mobileArea) {
        mobileArea.innerHTML = '<a href="/dashboard" class="btn-signin">Account</a>' +
      ...
      if (mobileArea) {
        mobileArea.innerHTML = '<a href="/login" class="btn-signin">Sign In</a>' +
  ...
  function wireAuthTrigger() {
    ...
    trigger.addEventListener('click', function() {
      if (window.FV.user) window.location.href = '/dashboard';
  ...
  if (window.FV.token) {
    fetch('/api/auth/me', {
      headers: { 'Authorization': 'Bearer ' + window.FV.token }
    })
    .then(function(r) {
      if (r.status === 401) {
        localStorage.removeItem('user_token');
